...
export async function startRemoteTrx(req: IncomingMessage, res: ServerResponse) {
    try {
        ...
        logger.info(`[API] Station ${chargePointId} is offline, creating simulated transaction`);
        ...
        // Добавляем в память
        connectionManager.addRecentTransaction(simulatedTransaction);
...
export async function stopRemoteTrx(req: IncomingMessage, res: ServerResponse) {
    try {
        ...
        logger.info(`[API] Station ${chargePointId} is offline, completing simulated transaction ${transactionId}`);

        // Пытаемся найти транзакцию в памяти и обновить её
        const recentTransactions = connectionManager.getRecentTransactions();
