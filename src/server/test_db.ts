...
function log(title: string, payload?: unknown) {
...
async function startSession(chargePointId: string, connectorId: number, idTag: string) {
    ...
    return resp.data?.transactionId as string | undefined;
}

async function stopSession(transactionId: string, chargePointId: string, connectorId: number, idTag: string) {
...
async function getTransactions(chargePointId?: string) {
...
async function getMetrics(chargePointId: string, from?: string, to?: string) {
...
async function reserve(chargePointId?: string, connectorId?: number, idTag?: string, expiryMinutes = 5) {
...
async function main() {
    ...
    if (!txId) throw new Error('Failed to get transactionId from start-session');
