...
const logger_1 = require("../logger"); // Адаптируйте путь к вашему logger
// Инициализация AJV
...
function validateMessage(payload, schemaName, isRequest = true) {
    const suffix = isRequest ? '' : 'Response';
    const fullName = schemaName + suffix;
    const validator = validators[fullName];
    if (!validator) {
        logger_1.logger.warn(`No schema defined for ${fullName} — skipping validation`);
