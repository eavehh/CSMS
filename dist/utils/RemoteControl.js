...
function sendRemoteMessage(connectionManager, chargePointId, action, payload) {
    ...
    if (!ws) {
        logger_1.logger.error(`[remoteControl] Нет связи с ${chargePointId}`);
        return;
    }
    const uniqueId = (0, uuid_1.v4)();
    const message = [2, uniqueId, action, payload];
    ws.send(JSON.stringify(message));
    logger_1.logger.info(`[remoteControl] Отправил ${action} на ${chargePointId}: ${JSON.stringify(payload)}`);
}
